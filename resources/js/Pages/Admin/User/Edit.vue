<template>
    <v-container v-if="form.id">
        <h1>ユーザ更新(<span>ID: {{ form ? form.id : '' }}</span>)</h1>
...
export default {
    async created() {
        this.form = _.cloneDeep(await this.getUser(this.$route.params.id));
        this.uploadImageUrl = this.$store.state.user.user.file_path;
    ...
    methods: {
        ...
        updateUser() {
            ...
            User.updateUser(data)
                .then(response => {
                    this.$router.push({ name: 'admin.user.detail', params :{ id: this.$route.params.id }});
